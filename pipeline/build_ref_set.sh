#! /bin/bash
####################################################################################################################
#THIS SCRIPT IS USED TO CREATE MULTIPLE SEQUENCE ALIGNMENTS OF 6 ARTHORPOD REFERENCE SPECIES.
#ONCE THE OUTPUT (ref_set.tgz) IS CREATED IT IS ADDED TO THE CONTAINER FOR COMPARISON TO EACH SPECIES OF INTEREST AT RUN TIME
#THE CURRENT SPECIES LIST INCLUDES: DROSOPHILA MELANOGASTER, APIS MELLIFERA, TRIBOLIUM CASTANEUM, PLODIA INTERPUNCTELLA, RHOPALOSIPHUM MAIDIS, SCHISTOCERCA SERIALIS CUBENSE AND ISCHNURA ELEGANS
#THESE SPECIES WERE SELECTED USING THIS COMMAND:
##	./datasets summary genome taxon "lepidoptera" --annotated --assembly-level complete,chromosome --as-json-lines | dataformat tsv genome --fields organism-name,accession,annotinfo-busco-complete,annotinfo-busco-singlecopy,annotinfo-busco-duplicated,annotinfo-busco-fragmented,annotinfo-busco-missing > lepidoptera.txt
## THIS WAS DONE FOR: LEPIDOPTERA, DIPTERA, HYMNEOPTERA, COLEOPTERA, HEMIPTERA, ORTHOPTERA, ODONATA
#EACH TSV OUTPUT FILE WAS RANKED FOR EACH BUSCO SCORE AND THE RANKS WERE SUMMED TO CHOOSE THE BEST SPECIES--**DROSOPHILA MELANOGASTER IS REQUIRED**
#THIS SCRIPT WILL NEED TO BE CHANGED AS GENOME ASSEMBLY/ANNOTATION UPDATES BECOME AVAILABLE
####################################################################################################################
mkdir ref_set
cd ref_set
...
mv dromel_cluster_renamed.fa dromel_cluster.fa
...
cpus=$(( $avail - 1 ))
orthofinder -f . -t $cpus

#COLLECT AND COMPRESS NECESSARY FILES
mv OrthoFinder/Results_*/WorkingDirectory .
mv OrthoFinder/Results_*/MultipleSequenceAlignments .
rm -r OrthoFinder *_cluster.fa
cd ../
tar -czf ref_set.tgz ref_set/
rm -r ref_set/
